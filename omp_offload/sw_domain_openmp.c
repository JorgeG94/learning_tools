...
anuga_int _openmp_backup_conserved_quantities(const struct domain *__restrict D)
{
  ...
  map(from:D->xmom_centroid_values[0:N])\
  map(from:D->ymom_centroid_values[0:N])\
  map(from:D->stage_backup_values[0:N])\
  map(from:D->xmom_backup_values[0:N])\
  map(from:D->ymom_backup_values[0:N])
