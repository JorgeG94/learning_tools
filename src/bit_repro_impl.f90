!> Implementations and private helpers for the bit_repro module.  Nothing in the
!! contains section below except the five `module function`s is visible outside
!! this submodule.  Bodies are verbatim from the pre-submodule layout -- the
!! restructure is bit-neutral by construction and gated by the harness checksums.
submodule (bit_repro) bit_repro_impl
