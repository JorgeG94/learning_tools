...
Targets (--target):
  ...
  cpu            Host multicore. The compute loop becomes `!$omp parallel do`
                 ...
                 Build the result with your host-OpenMP configuration
                 (no device offload).
...
def process_file(path: Path, write: bool, stats: Counter, warnings: list[str],
    ...
    try:
        ...
        warnings.append(f"{path}: unreadable, skipped")
        return 0
...
def main() -> int:
    ...
    order = ("enter_data", "exit_data", "data", "end_data", "update",
             ...
             "cpu_dropped", "cpu_clause_stripped")
    for k in order:
        v = stats.get(k, 0)
        if v:
            print(f"  {k:<20} {v:5d}")
