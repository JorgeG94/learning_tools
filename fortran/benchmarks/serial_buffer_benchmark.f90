program loop_order_sweep_serial_safe
  ...
  print *, "===================================================="
  print *, " Safe SERIAL benchmark (double-buffered)"
  ...
  do idx = 1, ntests
     ...
     !---------------------------------------------------------
     ! ✅ Verify correctness
     diff = maxval(abs(Anew - Bnew))
     diff = max(diff, maxval(abs(Anew - Cnew)))
     diff = max(diff, maxval(abs(Anew - Dnew)))
...
end program loop_order_sweep_serial_safe
