program blocked_dgemm
  ...
  real(real64), allocatable :: Awrk(:,:,:,:)  ! (Bsize, Bsize, Nblk, Mblk)
  ...
  allocate(Awrk(Bsize, Bsize, Nblk, Mblk))
  allocate(Bwrk(Bsize, Bsize, Nblk, Mblk))
  ...
  do concurrent (ib = 0:Nblk-1, jb = 0:Mblk-1)
    do kb = 0, Pblk-1
      ! Copy block of A - use Awrk(:,:,ib+1,jb+1)
      do concurrent (i=ib*Bsize:(ib+1)*Bsize-1, k=kb*Bsize:(kb+1)*Bsize-1)
        iloc = mod(i, Bsize) + 1
        kloc = mod(k, Bsize) + 1
        Awrk(iloc, kloc, ib+1, jb+1) = A(k+1, i+1)
      end do
      
      ! Copy block of B - use Bwrk(:,:,ib+1,jb+1)
      do concurrent (j=jb*Bsize:(jb+1)*Bsize-1, k=kb*Bsize:(kb+1)*Bsize-1)
        kloc = mod(k, Bsize) + 1
        jloc = mod(j, Bsize) + 1
        Bwrk(kloc, jloc, ib+1, jb+1) = B(j+1, k+1)
      end do
      ...
      do concurrent (i=ib*Bsize:(ib+1)*Bsize-1, j=jb*Bsize:(jb+1)*Bsize-1)
        ...
        do k = kb*Bsize, (kb+1)*Bsize-1
          kloc = mod(k, Bsize) + 1
          C(j+1, i+1) = C(j+1, i+1) + Awrk(iloc, kloc, ib+1, jb+1) * Bwrk(kloc, jloc, ib+1, jb+1)
        end do
      end do
    end do
  end do
